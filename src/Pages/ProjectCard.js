...
export default function ProjectCard(props) {
    ...
    return (
        ...
        <Grid item lg={8} xs={11} sm={9} md={9} container alignItems={'center'} justifyContent="space-between" style={styles.cardStyle} className={index && (index % 2 == 0 ? "animate__animated animate__fadeInRight" : "animate__animated animate__fadeInLeft") || "animate__animated animate__fadeInRight"}>
            <Grid item lg={6} xs={12} sm={6} md={6} container gap={4} p={3} justifyContent="center">
                <Grid item lg={11} xs={12} sm={10} md={10} container gap={4} p={3}>
                    <Typography variant='h5' fontWeight={'bold'}>{name}</Typography>
                    <Typography>{description}</Typography>
                    <Button variant='outlined' sx={{ borderRadius: 17 }} href={link}>View Project</Button>
                </Grid>
            </Grid>
            <Grid item lg={5} xs={12} sm={5} md={5} >
                <img src={image} width="100%" style={{ marginBottom: -5 }} />
            </Grid>
            {/* </Grid> */}
        </Grid>)
}
